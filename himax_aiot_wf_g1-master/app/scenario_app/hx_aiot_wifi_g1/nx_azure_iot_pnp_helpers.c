...
//#include <azure_sphere/nx_azure_iot_pnp_helpers.h>
#include <hx_aiot_wifi_g1/inc/nx_azure_iot_pnp_helpers.h>
#include <nx_api.h>
#include <stdio.h>
...
static UINT sample_json_child_token_move(NX_AZURE_IOT_JSON_READER *json_reader_ptr,
                                         UCHAR *property_name_ptr, UINT property_name_len)
{
    ...
    {
        if ((nx_azure_iot_json_reader_token_type(json_reader_ptr) ==
                NX_AZURE_IOT_READER_TOKEN_PROPERTY_NAME) &&
            nx_azure_iot_json_reader_token_is_text_equal(json_reader_ptr,
                                                         property_name_ptr,
                                                         property_name_len))
        {
    ...
    while (nx_azure_iot_json_reader_next_token(json_reader_ptr) == NX_AZURE_IOT_SUCCESS)
    {
        if (nx_azure_iot_json_reader_token_type(json_reader_ptr) == NX_AZURE_IOT_READER_TOKEN_PROPERTY_NAME)
        {
            ...
            sample_desired_property_callback(component_name_ptr, component_name_len,
                                             scratch_buf, len, *json_reader_ptr, version, context_ptr);
...
UINT nx_azure_iot_pnp_helper_command_name_parse(const UCHAR *method_name_ptr, UINT method_name_length,
...
UINT nx_azure_iot_pnp_helper_twin_data_parse(NX_AZURE_IOT_JSON_READER *json_reader_ptr, UINT is_partial,
                                             CHAR **sample_components_ptr, UINT sample_components_num,
                                             UCHAR *scratch_buf, UINT scratch_buf_len,
                                             VOID (*sample_desired_property_callback)(UCHAR *component_name_ptr,
...
UINT status;
    ...
    if (!is_partial && sample_json_child_token_move(json_reader_ptr,
                                                    (UCHAR *)sample_iot_hub_twin_desired,
                                                    sizeof(sample_iot_hub_twin_desired) - 1))
    {
    ...
    while (nx_azure_iot_json_reader_next_token(json_reader_ptr) == NX_AZURE_IOT_SUCCESS)
    {
        if (nx_azure_iot_json_reader_token_type(json_reader_ptr) ==
                NX_AZURE_IOT_READER_TOKEN_PROPERTY_NAME)
        {
            if (nx_azure_iot_json_reader_token_string_get(json_reader_ptr, scratch_buf,
                                                          scratch_buf_len, &len))
            {
                                       ...
                                       &index) == NX_AZURE_IOT_SUCCESS))
            {
                if (visit_component_properties((UCHAR *)sample_components_ptr[index],
                                               ...
                                               json_reader_ptr, version, scratch_buf, scratch_buf_len,
            ...
            {
                sample_desired_property_callback(NX_NULL, 0, scratch_buf, len,
                                                 *json_reader_ptr, version, context_ptr);
...
UINT nx_azure_iot_pnp_helper_build_reported_property(UCHAR *component_name_ptr, UINT component_name_len,
                                                     UINT (*append_reported_property)(NX_AZURE_IOT_JSON_WRITER *json_builder_ptr,
                                                                                      VOID *context),
                                                     VOID *context, NX_AZURE_IOT_JSON_WRITER *json_builder_ptr)
{
UINT status;

    if (nx_azure_iot_json_writer_append_begin_object(json_builder_ptr) ||
        (component_name_ptr != NX_NULL &&
         (nx_azure_iot_json_writer_append_property_name(json_builder_ptr,
                                                        component_name_ptr,
                                                        component_name_len) ||
...
UINT nx_azure_iot_pnp_helper_build_reported_property_with_status(UCHAR *component_name_ptr, UINT component_name_len,
                                                                 UCHAR *property_name_ptr, UINT property_name_len,
                                                                 UINT (*append_value)(NX_AZURE_IOT_JSON_WRITER *builder,
                                                                                      VOID *context),
                                                                 VOID *context,
                                                                 ...
                                                                 NX_AZURE_IOT_JSON_WRITER *json_builder_ptr)
{
    ...
    if (nx_azure_iot_json_writer_append_property_name(json_builder_ptr,
